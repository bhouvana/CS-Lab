...
fn exactly_one_leader_at_a_time() {
    let mut cluster = Cluster::new(5);
    cluster.run(200);
    let leader_count = cluster.nodes.iter().enumerate().filter(|(i, n)| cluster.alive[*i] && n.role == Role::Leader).count();
...
fn killing_the_leader_triggers_a_new_election() {
    ...
    let second_leader = cluster.leader().expect("a new leader should emerge after the old one is killed");
...
fn committed_command_replicates_to_a_majority() {
    ...
    let committed_count = (0..cluster.nodes.len())
        ...
        .count();
    assert!(committed_count * 2 > cluster.nodes.len(), "leader {leader}: only {committed_count}/5 nodes have the entry");
}
